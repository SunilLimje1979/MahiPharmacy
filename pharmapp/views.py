...
#so while integrating api instead of requests.post() use session.post(api_url,parameter)
...
def Login(request):
    if request.method == "POST":
        ...
        api_res = session.post('https://mahi-durg.app/medicalrecord/api/pharmacistLogin/',json={'username':username,'password':password})
        print(api_res.text)
...
def index(request):
    if('user' in request.session):
        api_res=session.post('https://mahi-durg.app/medicalrecord/api/get_pharmacist_stats/',json={'pharmacist_id':request.session.get('user').get('pharmacist_id')})
        print(api_res.text)
...
def approvedDoctor(request):
    if('user' in request.session):
        api_res = session.post('https://mahi-durg.app/medicalrecord/api/get_pharmacist_doctor_bypharmacistid/',json={'pharmacist_id':request.session.get('user').get('pharmacist_id')})
        print(api_res.text)
        if(api_res.json().get('message_code')==1000):
...
def get_patient_under_doctor(request ,id):
    ...
    api_res = session.post('https://mahi-durg.app/medicalrecord/api/get_patientdetails_by_doctor_pharmacist_id/',json={'doctor_id':doctor_id,'pharmacist_id':pharmacist_id})
    print(api_res.text)
    if(api_res.json().get('message_code')==1000):
...
def update_pharma_status(request):
    if request.method == 'POST':
        try:
            ...
            api_response = session.post('https://mahi-durg.app/medicalrecord/api/update_pharma_status/', json={
...
def showDeals(request):
    if('user' in request.session):
        res = session.post('https://mahi-durg.app/masters/api/get_active_deals_by_visible_to/',json={"visible_to":"2","user_id":request.session.get('user').get('pharmacist_id')}) #here '2' pass as a string means Pharmacy
        print(res.text)
